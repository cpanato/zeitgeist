...
type Container struct {
	...
	// Container URL, e.g. gcr.io/k8s-staging-kubernetes/conformance
	URL string
...
func (upstream Container) LatestVersion() (string, error) { // nolint:gocritic
	...
	return latestContainerVersion(&upstream)
}

func latestContainerVersion(upstream *Container) (string, error) {
	return latestContainerTag(upstream)
}

func latestContainerTag(upstream *Container) (string, error) {
	...
	log.Debugf("Retrieving tags for %s...", upstream.URL)
	tags, err := client.ListTags(upstream.URL)
	if err != nil {
